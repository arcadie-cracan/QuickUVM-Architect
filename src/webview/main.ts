...
/** Select the given PORT pins in the diagram (the inspector's "unmapped ports"
 *  list clicks through to them). Diagram behaviour, so it stays with the diagram
 *  and reaches the inspector as `ctx.onSelectPins`. */
...
function persistState(): void {
  vscode.setState({
    viewId: state.viewId,
    mode: state.mode,
    // the inspector's disclosures are persisted by inspector-view.ts, which owns
    // them; re-reading here would clobber them with a stale copy
    openSections: vscode.getState()?.openSections,
  });
}
